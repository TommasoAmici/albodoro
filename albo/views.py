...
def index(request):
    ...
    return render(
        request, "albo/albo.html", {"trophies": trophies, "comps_menu": comps_menu}
    )
...
def game_detail(request, pk):
    ...
    comps_menu = Competition.objects.filter(date__year=datetime.now().year)
    return render(
        request, "albo/partita.html", {"game": game, "comps_menu": comps_menu}
    )
...
def comps_all(request):
    ...
    comps_menu = Competition.objects.filter(date__year=datetime.now().year)
    return render(
        request, albo / comps_all.html, {"comps": comps, "comps_menu": comps_menu}
    )


def comp_detail(request, pk):
    comp = get_object_or_404(Competition, pk=pk)
    m = Standings.objects.filter(competition=comp).aggregate(Max("matchday"))[
        "matchday__max"
    ]
    try:
